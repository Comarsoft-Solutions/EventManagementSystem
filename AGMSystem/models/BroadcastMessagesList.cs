...
{
    public class BroadcastMessagesList : ErrorsLog
    {
        #region "Variables"
        ...
        public DataSet getUnassignedContacts(int BroadcastListID)
        {
            try
            {
                ...
                string str = "select pp.Id,pp.LastName + ' ' + pp.FirstName + ' ' + pp.Email as Member ,pp.Email from RegistrationMembers pp  where Email like '%@%' and pp.Id not in (select distinct(MemberID) from BroadcastListContacts where BroadCastListID = " + BroadcastListID + " )  order by pp.Id ";
                return ReturnDs(str);
            }
            catch (Exception ex)
            {
